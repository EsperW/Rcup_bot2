...
hsv_max_blue = np.array((159, 255, 255), np.uint8) # 180, 255,255
...
for i in range(0, 4):   # Перебор камер (0 ... 4)
   ...
   #sys.exit()
    time.sleep(1)
    ret, img = cap.read()
    print('Камера готова? -->', ret)
    if ret == True:
        break
    elif (i ==3 ):
        print("Ни одна из камер не подключена. Подключите камеру!!!")
        sys.exit()
...
# Детектирование и подсчет количества перекрестков
flagX = True   # Крестообразный перекресток
flagTH = True  # Т-образный перекресток
flagTL = True  # Т-образный перекресток, повернутый налево
flagTR = True  # Т-образный перекресток, повернутый направо
count_crossroads = 0
stop = 0
...
while True:
    ...
    img_red = np.copy(img)    # копирование 
    img_red[:400,:,:] = 255     # замаскирование верхней части для  красного цвета
    ...
    thresh_red_1 = cv2.inRange(hsv_red, hsv_min_red_1, hsv_max_red_1)
    thresh_red_2 = cv2.inRange(hsv_red, hsv_min_red_2, hsv_max_red_2)
    ...
    if (x_left != 0 and y_left != 0 and
        ...
        # print("X Поворот -->", rotation, "len(Green) --> ",len(Green))
    ...
    if len(Blue) != 0:
        ...
        # msg = "<"+"1"+","+ str(0) + "," + str(d_min_object) + "," + str(alpha) + "," + str(0) + "," + str(count_crossroads) + ">"
    ...
    if abs(y_cross - int(Height/2)) <= 35 and len(Red) == 0:
        rotation_send = rotation
        # Подсчет количества перекрестков
        if (flagX and
            x_left != 0 and y_left != 0 and
            x_right != 0 and y_right != 0 and
            x_top != 0 and y_top !=0 and
            x_bottom != 0 and y_bottom != 0):

            count_crossroads += 1
            flagX = False  # Крестообразный перекресток
            flagTH = True  # Т-образный перекресток
            flagTL = True  # Т-образный перекресток, повернутый налево
            flagTR = True  # Т-образный перекресток, повернутый направо
        ...
        # Подсчет количества перекрестков
        if (flagTH and
            (x_left != 0 or y_left != 0) and
            (x_right != 0 or y_right != 0) and
            (x_bottom != 0 or y_bottom != 0) and
            (x_top == 0 or y_top == 0)):

            count_crossroads += 1
            flagX = True  # Крестообразный перекресток ВНИЗ
            flagTH = False  # Т-образный перекресток
            flagTL = True  # Т-образный перекресток, повернутый налево
            flagTR = True  # Т-образный перекресток, повернутый направо
        ...
        if (flagTL and
            (x_left != 0 or y_left != 0) and
            (x_top != 0 or y_top != 0) and
            (x_bottom != 0 or y_bottom != 0) and
            (x_right == 0 or y_right == 0)):

            count_crossroads += 1
            flagX = True  # Крестообразный перекресток ВНИЗ
            flagTH = True  # Т-образный перекресток
            flagTL = False  # Т-образный перекресток, повернутый налево
            flagTR = True  # Т-образный перекресток, повернутый направо
        ...
        if (flagTR and
            ...
            (x_top != 0 or y_top != 0) and
            (x_bottom != 0 or y_bottom != 0) and
            (x_left == 0 or y_left == 0)):

            count_crossroads += 1
            flagX = True  # Крестообразный перекресток ВНИЗ
            flagTH = True  # Т-образный перекресток
            flagTL = True  # Т-образный перекресток, повернутый налево
            flagTR = False  # Т-образный перекресток, повернутый направо

        # msg = "<"+"1"+","+str(rotation) + "," + str(d_min_object) + "," + str(alpha) + "," + str(0) + "," + str(count_crossroads) +">"
        # print("<== Поворот ==>",  rotation)
        # SEND_MESANGE_TO_COM_PORT_2(msg)

    if len(Red) != 0 and Red[0][1] > int(Height/2) + 60 and  Red[0][1] <= Height:
        stop = 1
        # msg = "<"+"1"+","+str(0) + "," + str(0) + "," + str(0) + "," + str(1) + "," + str(count_crossroads) + ">"

    msg = "<" + "1" + "," + str(rotation_send) + "," + str(d_min_object) + "," + str(alpha) + "," + str(stop) + "," + str(count_crossroads) + ">"
    SEND_MESANGE_TO_COM_PORT_2(msg)
